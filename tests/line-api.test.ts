...
describe('pushLineMessage', () => {
  ...
  test('keeps cornerRadius on boxes, where LINE supports it (rounded pills and panels)', async () => {
    ...
    const pill = {
      ...
      cornerRadius: '20px',
      borderWidth: '1px',
      contents: [{ type: 'text', text: 'ดูโปร' }],
    }
    await pushLineMessage('U123', [{
      type: 'flex',
      altText: 'promo',
      contents: { type: 'bubble', body: { type: 'box', layout: 'vertical', contents: [pill] } },
    }], 3)

    const body = JSON.parse((global.fetch as any).mock.calls[0][1].body)
    expect(body.messages[0].contents.body.contents[0]).toEqual(pill)
  })
