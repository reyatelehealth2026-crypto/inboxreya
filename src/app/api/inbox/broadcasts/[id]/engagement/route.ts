...
const ENGAGEMENT_EVENTS = ['impression', 'postback', 'conversion', 'click'] as const
const ENGAGEMENT_SOURCES = ['liff', 'postback', 'webhook', 'redirect'] as const
...
export async function GET(req: NextRequest, { params }: RouteContext) {
  try {
    ...
    const [events, total, summaryRows, uniqueByEvent, clickRegionRows, uniqueClickerRows, anonymousClicks] = await Promise.all([
      ...
      prisma.broadcastEngagement.groupBy({
        by: ['action'],
        where: { broadcastId, eventType: 'click' },
        _count: { _all: true },
      }),
      prisma.broadcastEngagement.findMany({
        where: { broadcastId, eventType: 'click', lineUserId: { not: 'anon' } },
        distinct: ['lineUserId'],
        select: { lineUserId: true },
      }),
      prisma.broadcastEngagement.count({
        where: { broadcastId, eventType: 'click', lineUserId: 'anon' },
      }),
    ])
    ...
    for (const row of clickRegionRows) {
      if (row.action) clicksByRegion[row.action] = row._count._all
    }
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        summary: {
          ...
          uniqueUsersByEvent,
          clicksByRegion,
          uniqueClickers: uniqueClickerRows.length,
          anonymousClicks,
        },
        events: rows,
        pagination: {
