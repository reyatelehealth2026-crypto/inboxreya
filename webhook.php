...
function handleMessage($event, $userId, $replyToken, $db, $line, $lineAccountId = null)
{
    try {
        ...
        if (in_array($messageType, ['image', 'video', 'audio', 'file'])) {
            ...
            if ($messageType === 'image' && $userState && in_array($userState['state'], ['waiting_slip', 'awaiting_slip'])) {
                ...
                if ($orderId) {
                    ...
                    $slipMessageId = $db->lastInsertId();

                    // Sync slip message to Next.js API (non-blocking)
                    try {
        ...
        // Sync message to Next.js API (non-blocking)
        try {
            if (function_exists('syncMessageToNextjs')) {
            ...
            error_log('Next.js sync failed: ' . $e->getMessage());
        }
...
function saveOutgoingMessage($db, $userId, $content, $sentBy = 'system', $messageType = 'text')
{
    try {
        ...
        // Sync outgoing message to Next.js API (non-blocking)
        try {
            if (function_exists('syncMessageToNextjs')) {
                // Get user's line_user_id and line_account_id for sync
                $userStmt = $db->prepare("SELECT line_user_id, line_account_id, display_name, picture_url FROM users WHERE id = ?");
                $userStmt->execute([$userId]);
                $userData = $userStmt->fetch(PDO::FETCH_ASSOC);

                if ($userData && $userData['line_user_id']) {
                    syncMessageToNextjs($userData['line_user_id'], $userData, [
                        'id' => $messageId,
                        'direction' => 'outgoing',
                        'type' => $messageType,
                        'content' => $contentStr,
            ...
            // Log error but don't fail the webhook
            error_log('Next.js sync failed (outgoing): ' . $e->getMessage());
        }
...
/**
 * Sync message to Next.js API
 * ส่งข้อมูล message ไปยัง Next.js API endpoint สำหรับ real-time updates
 */
function syncMessageToNextjs($lineUserId, $user, $messageData, $lineAccountId = null)
{
    // ตรวจสอบว่า config ถูกตั้งค่า
    if (!defined('NEXTJS_API_URL') || !defined('INTERNAL_API_SECRET')) {
        return false;
    }
    ...
    // ถ้าไม่มี URL หรือ secret ให้ skip
    if (empty($nextjsApiUrl) || empty($internalSecret)) {
        return false;
    }

    try {
        // เตรียม payload ตาม format ที่ Next.js API ต้องการ
        $payload = [
            'event' => 'message',
            'data' => [
        ...
        // ส่ง request ไปยัง Next.js API
        $ch = curl_init($nextjsApiUrl . '/api/sync/webhook');
        curl_setopt_array($ch, [
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_POST => true,
            ...
            CURLOPT_TIMEOUT => 5, // Timeout 5 seconds เพื่อไม่ให้ชะลอ webhook
        ...
        // ตรวจสอบผลลัพธ์
        if ($httpCode === 200) {
            // Success - log ถ้า debug mode
            if (defined('DEBUG_MODE') && DEBUG_MODE) {
                $messageId = isset($messageData['id']) ? $messageData['id'] : 'N/A';
                error_log("Next.js sync successful: lineUserId={$lineUserId}, messageId={$messageId}");
            }
            return true;
        } else {
            // Log error แต่ไม่ throw exception
            error_log("Next.js sync failed: HTTP {$httpCode}, lineUserId={$lineUserId}, error={$curlError}, response=" . substr($response, 0, 200));
            return false;
        }
    } catch (Exception $e) {
        // Log error แต่ไม่ throw exception เพื่อไม่ให้ webhook fail
        error_log("Next.js sync exception: " . $e->getMessage() . ", lineUserId={$lineUserId}");
        return false;
    }
}
