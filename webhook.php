    ...
    try {
                ...
                try {
                    ...
                    if ($fileData && strlen($fileData) > 0) {
                        ...
                        if (file_put_contents($filepath, $fileData)) {
                            ...
                            $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https://' : 'http://';
                            $host = $_SERVER['HTTP_HOST'] ?? (defined('BASE_URL') ? parse_url(BASE_URL, PHP_URL_HOST) : 'localhost');
                            $savedMediaUrl = $protocol . $host . '/uploads/line_files/' . $filename;
                ...
                try {
                    ...
                    if ($audioData && strlen($audioData) > 0) {
                        ...
                        if (file_put_contents($filepath, $audioData)) {
                            ...
                            $protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https://' : 'http://';
                            $host = $_SERVER['HTTP_HOST'] ?? (defined('BASE_URL') ? parse_url(BASE_URL, PHP_URL_HOST) : 'localhost');
                            $savedMediaUrl = $protocol . $host . '/uploads/line_audio/' . $filename;
        ...
        if ($messageType === 'file') {
            if (!empty($originalFileName)) {
                $metadataArr['fileName'] = $originalFileName;
            }
            if (!empty($fileSize)) {
                $metadataArr['fileSize'] = $fileSize;
            }
        }
        ...
        try {
            ...
            // Check media_url
            if (!empty($savedMediaUrl)) {
                $check = $db->query("SHOW COLUMNS FROM messages LIKE 'media_url'");
                if ($check->rowCount() > 0) {
                    $cols[] = 'media_url';
                    $vals[] = $savedMediaUrl;
                    $placeholders[] = '?';
...
function sendTelegramNotificationWithMedia($db, $line, $displayName, $messageType, $messageContent, $messageId, $dbUserId, $messageData)
{
    ...
    if ($messageType === 'image') {
        ...
        $fileName = $messageData['fileName'] ?? ($messageContent ?: "ID: {$messageId}");
        $telegram->notifyNewMessage($displayName, "[ไฟล์] {$fileName}", '', $dbUserId);
    } elseif ($messageType === 'video') {
